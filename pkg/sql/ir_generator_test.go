...
`
	...
	a.Equal("SUM((price - materials_cost - other_cost) * product)", stmt.Objective.Expression)
	a.Equal("", stmt.Objective.GroupBy)

	a.Equal(3, len(stmt.Constraints))
	a.Equal("SUM(finishing * product) <= 100", stmt.Constraints[0].Expression)
	a.Equal("", stmt.Constraints[0].GroupBy)
	a.Equal("SUM(carpentry * product) <= 80", stmt.Constraints[1].Expression)
	a.Equal("", stmt.Constraints[1].GroupBy)
	a.Equal("product <= max_num", stmt.Constraints[2].Expression)
...
`
	...
	a.Equal("SUM(distance * shipment * 90 / 1000)", stmt.Objective.Expression)
	a.Equal("", stmt.Objective.GroupBy)

	a.Equal(2, len(stmt.Constraints))
	a.Equal("SUM(shipment) <= capacity", stmt.Constraints[0].Expression)
	a.Equal("plants", stmt.Constraints[0].GroupBy)
	a.Equal("SUM(shipment) >= demand", stmt.Constraints[1].Expression)
