...
type Expr struct {
	...
	// the printing form is in val; otherwise, token and val are ignorable,
	// but sexp it is a Lisp S-expression and funcall indicate if sexp is a
	// function call or usual unary or binary expressions.  funcall is
	// useful for printing the expression.  We should not required funcall
	// because if sexp is a function call, sexp[0].token must be IDENT.
	// However, goyacc cannot allow us to use externally defined constant
	// symbol IDs in *.y files.  And the *.y file needs to import package
	// ast, so we cannot have ast to import package parser so to use symbol
	// IDs generated by goyacc.  This constaint forces us to add this ad-hoc
	// field funcall.
	token   int
	val     string
	sexp    ExprList
...
func NewAtomic(token int, val string) (*Expr, error) {
	if token == 0 {
