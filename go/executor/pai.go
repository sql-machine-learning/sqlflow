...
func achieveResource(cwd, entryCode, requirements, tarball, estimator string) error {
	...
	// sqlflow_runtime and sqlflow_models are built-in packages.
	if err := copyPythonPackage("sqlflow_runtime", cwd); err != nil {
		return err
	}
	...
	cmd := exec.Command("tar", "czf", tarball, "./sqlflow_runtime", "./sqlflow_models", entryFile, "requirements.txt")
	cmd.Dir = cwd
	if _, err := cmd.CombinedOutput(); err != nil {
		return fmt.Errorf("failed %s, %v", cmd, err)
	}
