...
func TestGuessError(t *testing.T) {
	...
	a.Error(err)
	a.Contains(err.Error(), "PAI task failed, please go to check details error logs in the LogViewer website: ")
	...
	a.Error(err)
	a.Contains(err.Error(), "PAI task failed, due to lack of the auth for PAI to access OSS(need to contact your administrator), please go to")
