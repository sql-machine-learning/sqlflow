...
var ChurnSQL = fmt.Sprintf(`CREATE DATABASE IF NOT EXISTS churn;
...
INSERT INTO churn.train VALUES %s;
...
INSERT INTO churn.test VALUES %s;`,
	churnTrainData, churnTestData)

// ChurnHiveSQL is .sql format data sample of churn dataset in Hive SQL.
var ChurnHiveSQL = fmt.Sprintf(`CREATE DATABASE IF NOT EXISTS churn;
DROP TABLE IF EXISTS churn.train;
CREATE TABLE churn.train (
	customerID VARCHAR(255),
	gender VARCHAR(255),
...
INSERT INTO TABLE churn.train VALUES %s;
...
INSERT INTO TABLE churn.test VALUES %s;`,
	churnTrainData, churnTestData)

// ChurnMaxComputeSQL is .sql format data sample of churn dataset in MaxCompute SQL.
// TODO(sneaxiy): add test data of churn dataset if needed
var ChurnMaxComputeSQL = `DROP TABLE IF EXISTS %[1]s.sqlflow_test_churn_train;
CREATE TABLE %[1]s.sqlflow_test_churn_train (
	customerID STRING,
	gender STRING,
	SeniorCitizen INT,
	Partner STRING,
...
INSERT INTO TABLE %[1]s.sqlflow_test_churn_train VALUES` + churnTrainData + ";" // cannot use fmt.Sprintf here
