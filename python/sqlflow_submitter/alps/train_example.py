...
class SQLFlowEstimatorBuilder(EstimatorBuilder):
    def _build(self, experiment, run_config):
        feature_columns = []
        feature_columns.append(tf.feature_column.embedding_column(tf.feature_column.categorical_column_with_identity(key="deep_id_0", num_buckets=15033), dimension=512, combiner="mean", initializer=None))
        feature_columns.append(tf.feature_column.embedding_column(tf.feature_column.categorical_column_with_identity(key="user_space_stat_0", num_buckets=310), dimension=64, combiner="mean", initializer=None))
        feature_columns.append(tf.feature_column.embedding_column(tf.feature_column.categorical_column_with_identity(key="user_behavior_stat_0", num_buckets=511), dimension=64, combiner="mean", initializer=None))
        feature_columns.append(tf.feature_column.embedding_column(tf.feature_column.categorical_column_with_identity(key="space_stat_0", num_buckets=418), dimension=64, combiner="mean", initializer=None))
        return tf.estimator.DNNClassifier(n_classes=2,hidden_units=[10,20],config=run_config,feature_columns=feature_columns)


if __name__ == "__main__":
    odps_project = os.getenv("MAXCOMPUTE_PROJECT")
    odps_conf = OdpsConf(
