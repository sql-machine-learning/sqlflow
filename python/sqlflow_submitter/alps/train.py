...
from alps.framework.engine import LocalEngine, YarnEngine, KubemakerEngine, ResourceConf
...
def train(model_builder,
          ...
          epochs=1, batch_size=1, shuffle=False, shuffle_bufsize=0,
          cache_file="",
          max_steps=None,
          eval_steps=None,
          eval_batch_size=1,
    ...
    if feature_map_table != "":
        feature_map = FeatureMap(table=feature_map_table, partition=feature_map_partition)
    else:
        feature_map = None
    ...
    if engine_config["name"] == "LocalEngine":
        ...
        engine = YarnEngine(cluster=engine_config["cluster"],
                            ...
                            ps = ResourceConf(memory=engine_config["ps_mem"], num=engine_config["ps_num"]),
                            worker=ResourceConf(memory=engine_config["worker_mem"], num=engine_config["worker_num"]))
    elif engine_config["name"] == "KubemakerEngine":
        engine = KubemakerEngine(cluster=engine_config["cluster"],
                            queue=engine_config["queue"],
                            ps = ResourceConf(memory=engine_config["ps_mem"], num=engine_config["ps_num"]),
                            worker=ResourceConf(memory=engine_config["worker_mem"], num=engine_config["worker_num"]))
    else:
    ...
    experiment = Experiment(
        ...
        exporter=ArksExporter(signature_def_key='predict', deploy_path=export_path, strategy=ExportStrategy.LATEST, compare=MetricComparator("auc", Goal.MAXIMIZE)),
