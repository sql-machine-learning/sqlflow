...
class HiveDBWriter(BufferedDBWriter):
    ...
    def write_hive_table(self):
        ...
        cmd_str = "hdfs dfs -fs hdfs://%s -mkdir -p %s/%s/" % (namenode_addr, hdfs_path, self.table_name)
        subprocess.check_output(cmd_str.split(), env=hdfs_envs)
        cmd_str = "hdfs dfs -fs hdfs://%s -copyFromLocal %s %s/%s/" % (namenode_addr, self.tmp_f.name, hdfs_path, self.table_name)
        subprocess.check_output(cmd_str.split(), env=hdfs_envs)
        # load CSV into Hive
        cursor = self.conn.cursor()
