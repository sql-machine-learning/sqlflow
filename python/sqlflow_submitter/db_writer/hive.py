...
class HiveDBWriter(BufferedDBWriter):
    ...
    def write_hive_table(self):
        ...
        cmd_str = "hdfs dfs -mkdir -p /%s%s/%s/" % (namenode_addr, hdfs_path, self.table_name)
        subprocess.check_output(cmd_str.split(), env=hdfs_envs)
        cmd_str = "hdfs dfs -copyFromLocal %s /%s%s/%s/" % (self.tmp_f.name, namenode_addr, hdfs_path, self.table_name)
        subprocess.check_output(cmd_str.split(), env=hdfs_envs)
        # load CSV into Hive
        cursor = self.conn.cursor()
        load_sql = "LOAD DATA INPATH '/%s%s/%s/' OVERWRITE INTO TABLE %s" % (
            namenode_addr,
            hdfs_path,
            self.table_name,
            self.table_name
