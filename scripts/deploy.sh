...
if [[ "$TRAVIS_BRANCH" == "develop" ]]; then
    ...
    if [[ "$TRAVIS_EVENT_TYPE" != "cron" ]]; then
        docker run --rm -e COVERALLS_TOKEN=$COVERALLS_TOKEN -v $PWD:/workspace sqlflow:latest bash -c "cd /workspace && goveralls -coverprofile=coverage.out -service=travis-ci -repotoken $COVERALLS_TOKEN"
    fi
