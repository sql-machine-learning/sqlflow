...
1. A **model collection** is a Docker image that packs a list model definitions.
...
### Release the Model Collection Docker Image
...
```bash
$ sqlflow release modelcoll my_awesome_model/ model_image:v0.1
...
If the model collection image `model_image:v0.1` already exists, the above command should fail. To force overwrite the model collection image you should run:

```bash
$ sqlflow release modelcoll -f my_awesome_model/ model_image:v0.1
```

When the model collection goes out of date, you may need to delete it:

```bash
$ sqlflow delete modelcoll model_image:v0.1
```

### Use the Model Collection Docker Image
...
By default, we use a default Docker image to run the training, predicting or explaining job. The default Docker image contains pre-made Tensorflow estimator models, Keras models defined in [sqlflow_models repo](https://github.com/sql-machine-learning/models) and XGBoost. To use a custom model collection Docker image, write SQL statements mentioned above:
...
- The model zoo server should be configured to connect to one Docker registry (e.g. hub.docker.com) together with the registry's repository and account. The model zoo server can accept below configuration file when starting:
    ...
    password=xxxx
    ```
- When releasing a model collection, the model zoo server will get the client uploaded file folder with a `Dockerfile` inside, then:
    ...
    1. Write a row in the MySQL instance to recored this model collection Docker image.
- When releasing a trained model, the model zoo server will:
...
- When deleting a model collection image or a trained model, do the reverse. **Note that we do not really delete the record in MySQL, yet we set the `deleted` flag so that the deleted model collection or trained model will not be listed. Then if some one is using a trained model trained by a deleted model collection, the trained model can still be available.**

### Model Zoo Server Database Schema

We create three tables using the schema described in the below figure:

1. Table `model_collection` stores model collection Docker images with it's corresponding version tags.
2. Table `model_definition_details` stores model collection details provided by each Docker image. Note that each Docker image may have multiple model definition classes or functions.
3. Table `trained_models` stores the unique name of the trained model and a URL point to where the trained model is actually saved. Note that each model collection can be used to train multiple trained models.
...
1. Develop model definitions in a Git repo as the "model collection", say, `github.com/a_data_scientist/regressors`.
1. Release the model collection to SQLFlow using the command `sqlflow release modeldef -f my_awesome_model/ model_image:v0.1`
