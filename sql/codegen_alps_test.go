...
func TestTrainALPSFiller(t *testing.T) {
	...
	a.True(strings.Contains(filler.X, "SparseColumn(name=\"deep\", shape=[2000], dtype=\"float\", separator=\",\")"), filler.X)
	a.True(strings.Contains(filler.X, "SparseColumn(name=\"wide\", shape=[1000], dtype=\"float\", separator=\",\")"), filler.X)
